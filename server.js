...
app.get("/api/meter/:custNo", async (req, res) => {
  try {
    const custNo = req.params.custNo;

    const response = await submitRequest(custNo, "রিচার্জ হিস্ট্রি");
...
app.get("/api/meter/:custNo/recharge-history/pdf", async (req, res) => {
  try {
...
app.get("/api/meter/:custNo/certificate/pdf", async (req, res) => {
  try {
    const response = await submitRequest(
      req.params.custNo,
